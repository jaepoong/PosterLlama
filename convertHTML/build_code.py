    ...
    def __init__(
        ...
        **kwargs
        ...
        if args.add_task_instruction:
            task_instruction = TASK_INSTRUCTION[args.dataset_name] # I want to generate layout in poster design format.
            self.cond_cate_to_size_pos = task_instruction + self.cond_cate_to_size_pos 
            # please generate the layout html according to the categories and image I provide (in html format):\n###bbox html: {bbox_html}
            self.cond_cate_size_to_pos = task_instruction + self.cond_cate_size_to_pos
            # please generate the layout html according to the categories and size and image I provide (in html format):\n###bbox html: {bbox_html}
            self.cond_random_mask = task_instruction + self.cond_random_mask
            # please recover the layout html according to the bbox, categories and size according to the image I provide (in html format):\n###bbox html: {bbox_html}
        self.cond_bbox_prefix=""
        self.cond_cate_prefix=""
        self.category_map = DATASET_META[dataset.dataset_name] # category matching
        self.glue_template_train_eval = SEP_SEQ[0]
        self.glue_template_test = SEP_SEQ[1]
        self.glue_template_codegen_train = SEP_SEQ[2]
        self.glue_template_codegen_test = SEP_SEQ[3] # <MID>
        
        self.tokenizer = tokenizer # llama tokenizer
        self.N_category = dataset.N_category # category num
