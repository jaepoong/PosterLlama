...
def metrics_uti(img_names, clses, boxes):
    ...
    for idx, name in enumerate(img_names):
        pic_1 = np.array(Image.open(os.path.join("data/PKU_PosterLayout/train/saliencymaps_pfpn", 
                                               name.split(".")[0]+"_pred."+name.split(".")[1])).convert("L").resize((513, 750))) / 255
        pic_2 = np.array(Image.open(os.path.join("data/PKU_PosterLayout/train/saliencymaps_basnet", 
...
def metrics_rea(img_names, clses, boxes):
    ...
    metrics = 0
    for idx, name in enumerate(img_names):
        pic = Image.open(os.path.join("data/PKU_PosterLayout/train/inpainted_poster", name)).convert("RGB").resize((513, 750))
...
def main():
    ...
    log_dir = "log_dir/train_stage2_with_augment_dino_codellama/generated_sample/16_f"
    print(log_dir)
    ...
    if cal_fid:
        ...
        fid = cal_layout_fid(fid_model,boxes,boxes_gt,clses,clses_gt,pku=True)
